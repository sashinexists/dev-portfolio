module Page.Tag.Name_ exposing (Data, Model, Msg, page)

import BlogPosts exposing (blogPostsWithTag)
...
routes =
    DataSource.Http.get (Secrets.succeed "https://cdn.contentful.com/spaces/gh3negosphjh/environments/master/tags?access_token=TY_E9VvxnyO2jK19-khEq6VbH_eqaDepbu4TzXGUNZU") decodeTags


decodeTags : Decoder (List RouteParams)
decodeTags =
    Decode.field "items" (Decode.list decodeTag)


decodeTag : Decoder RouteParams
decodeTag =
...
data routeParams =
    blogPostsWithTag routeParams.name
...
head static =
    Seo.summary
        ...
        , locale = Nothing
        , title = "Sashin Dev — Tag — " ++ static.routeParams.name -- metadata.title -- TODO
        }
        |> Seo.website
...
view maybeUrl sharedModel static =
    ...
    , body =
        [ case sharedModel.device.class of
            ...
            Shared.Phone ->
                viewPhonePage static.data static.routeParams
...
viewContent content routeParams =
    Element.column [ spacing 20, centerX, centerY, width <| px <| 768 ]
        [ viewTagPageTitle routeParams, viewBlogPosts content ]


viewTagPageTitle : RouteParams -> Element msg
viewTagPageTitle routeParams =
    Element.paragraph
        [ Font.size 30, Font.extraLight, centerX, centerY, paddingEach { top = 15, bottom = 5, right = 20, left = 20 } ]
        [ Element.text "Posts where I've written about "
        , Element.paragraph [ Font.color theme.navLinkHoverColor ] [ Element.text (slugToTitle routeParams.name) ]
        ]
...
viewPhonePage : Data -> RouteParams -> Element msg
viewPhonePage content routeParams =
    Element.column [ centerX, centerY, width fill ]
        [ viewPhoneContent content routeParams
        , viewFooter
        ]


viewPhoneContent : Data -> RouteParams -> Element msg
viewPhoneContent content routeParams =
    Element.column [ spacing 20, centerX, centerY ]
        [ viewPhoneTagPageTitle routeParams
...
viewPhoneTagPageTitle : RouteParams -> Element msg
viewPhoneTagPageTitle routeParams =
    Element.paragraph
        [ Font.size 20, Font.extraLight, centerX, centerY, paddingEach { top = 15, bottom = 5, right = 20, left = 20 } ]
        [ Element.text "Posts where I've written about "
        , Element.paragraph [ Font.color theme.navLinkHoverColor ] [ Element.text (slugToTitle routeParams.name) ]
        ]
